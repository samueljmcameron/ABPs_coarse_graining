...
temp_eff = Pswim/rho_num+1

axarr[0].plot(fps,Ploads*1000,'>-',color=colors[0],
              label=r'$P_L-P_{\mathrm{swim}}-P_{\mathrm{id}}$')
axarr[0].plot(fps,Pcalcs_rdf*1000*temp_eff,'s-',color=colors[2],
              label=r'$P_{ss}^{(2)}$')
axarr[0].plot(fps,Pcalcs_total*1000*temp_eff,'^-',color=colors[3],
...
axarr[1].plot(fps,Ploads,'>-',color=colors[0],
              label=r'$P_L-P_{\mathrm{swim}}-P_{\mathrm{id}}$')
axarr[1].plot(fps,Pcalcs_rdf*temp_eff,'s-',color=colors[2],
              label=r'$P_{ss}^{(2)}$')
axarr[1].plot(fps,Pcalcs_total*temp_eff,'^-',color=colors[3],
              label=r'$P_{ss}^{(2)}+P_{ss}^{(3)}$')
...
fig.suptitle(r"$\beta_{\mathrm{eff}}=(1+\mathrm{Pe}^2/(4\Pi))^{-1}$")
fig.subplots_adjust(left=0.2,hspace=0.05,top=0.85)
fig.savefig('results/eff_temp'
